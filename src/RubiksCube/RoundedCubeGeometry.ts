// https://dusanbosnjak.com/test/webGL/three-rounded-box/
...
export class RoundedCubeGeometry extends THREE.BufferGeometry {
  ...
  constructor(size = 1, radius = 0.12, radiusSegments = 1) {
    super();
    ...
    const doVertices = () => {
      ...
      const cornerOffset = new THREE.Vector3(edgeHalfSize, edgeHalfSize, edgeHalfSize);
      ...
      for (let y = 0; y <= radiusSegments; y++) {
        ...
        if (y == radiusSegments) {
          ...
          const vert = vertex.clone().multiplyScalar(radiusSize).add(cornerOffset);
...
export class RoundedPlaneGeometry extends THREE.ExtrudeBufferGeometry {
  ...
  constructor(size: number, radius: number, depth: number) {
    ...
    shape.quadraticCurveTo(x + width, y + height, x + width, y + height - radiusSize);
